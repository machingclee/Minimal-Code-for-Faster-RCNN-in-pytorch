...
class AnnotationDataset(Dataset):
    def __init__(self, mode="train"):
        ...
        annotation_files = ["datasets/coco_annotations.json"]
        # negative_sample_dir = "datasets/normal"
        self.data: List[Target] = []

        imageId_imagePath = {}
        self.cls_names = ["spalling", "crack"]
        ...
        for annotation_file in annotation_files:
            ...
            for cat in categories:
                cat_id = cat["id"]
                # cat_name = cat["name"]
                # self.cls_names.append(cat_name)
                self.cat_coco_ids.append(cat_id)
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join("datasets", data["image_path"])
        boxes = data["boxes"]
        boxes = np.array(boxes)

        img = Image.open(img_path.replace("/datasets", "datasets")
                         .replace("20210106_Sun_Tak_House_AE_CW_VIR/", "")
                         .replace("20210820_Olympic_Building_CW_VIR/", "")  
                         .replace("20210113_first_innocentre_AE_CW_V/", "")
                    )
        img = np.array(img)
